...
x1=ori.newNode(5,:)+squeeze(Uhis(:,5,:));
x2=ori.newNode(6,:)+squeeze(Uhis(:,6,:));
x3=ori.newNode(2,:)+squeeze(Uhis(:,2,:));
x4=ori.newNode(1,:)+squeeze(Uhis(:,1,:));
...
theta=zeros(500,1);
for i=1:500
    theta(i)=acos(dot(vec1(i,:),vec2(i,:))/norm(vec1(i,:))/norm(vec2(i,:)));
end

qhis=((1:500)/500*qload)';
