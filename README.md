...
Simulating active origami structures with multi-physics based realistic behaviors. 
...
different loading methods adn allows users to created sutomized loading schemes with 
...
* Provides five different loading simulation methods: Newton-Raphson method,
...
* Allows users to create customized loading schemes with arbitrary number and 
...
* Simulates heat transfer in origami systems, and can capture the electro-
...
**Figure 1.** The package allows users to simulate origami systems with compliant creases, which are creases with non-negligible crease. 
Using compliant creases provides more realistic geometry and allows the simulator to capture advanced mechanical behaviors such as bistability
 and multi-physics actuation introduced by these creases. 
...
**Figure 2.** The package allows users to simulate inter-panel contact within the origami. This panel contact model is a physics-based 
...
the heat transfer of active origami and calcualted the active crease folding due to changing temperature. 
...
active origami, please check the example codes in "02_ThermalLoadingExample". Additional 
