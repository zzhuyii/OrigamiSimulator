...
[oldCreaseNum,oldCreaseConnect,oldCreaseType]=...
    Mesh_IdentifyCrease(node0,panel0);
...
creaseWidthMat(3)=200*10^(-6); 
creaseWidthMat(6)=200*10^(-6); 
...
modelMechanicalConstant{2}=0.8*2*10^9; 
...
modelMechanicalConstant{11}=40*(10^(-6));

% d0center: d0 for points at the center
modelMechanicalConstant{12}=40*(10^(-6));
...
%% Residual Stress induced folding

% Define the analyses parameter for residual stress loading

assembleConstant=zeros(3,1);

% mechanical increStep in thermal step
assembleConstant(1)=10; 

% tor
assembleConstant(2)=4*10^-6; 
...
[U,Uhis,strainEnergyAssemble]=Solver_Assemble(panel0,newNode2OldNode,...
    newNode,barConnect,barType,barLength,barArea,...
...
residualStressShape=newNode+U;
Plot_DeformedShape(viewControl,newNode,residualStressShape,newPanel);
...
loadMag=(a*a*20*10^(-6)*1200*9.8)/loadConstant(1)/6;
load=[5,0,0,-loadMag;
      6,0,0,-loadMag;
      7,0,0,-loadMag;
      8,0,0,-loadMag;
...
loadMag=(a*a*20*10^(-6)*1200*9.8)/6;
load=[5,0,0,-loadMag;
      6,0,0,-loadMag;
      7,0,0,-loadMag;
      8,0,0,-loadMag;
...
modelThermalConstant{1}  = [1.3;0.3;0.3;1.3]; 
...
modelThermalConstant{4} = [1;1;1;1]*400*10^(-6); 

% Thickness of the submerged environment at RT for creases
modelThermalConstant{5} = 400*10^(-6); 
...
modelTimoshenkoConstant{2}= 0.7*79*10^9; 
...
qtotal=10/1000; % total input energy with unit W
qin=zeros((modelThermalConstant{6}+1)*thermalNodeNum,1);
qin(2)=1/4*1/2*qtotal;
qin(3)=1/4*1/2*qtotal;
qin(5)=1/4*1/2*qtotal;
qin(8)=1/4*1/2*qtotal;

qin(17)=1/2*1/6*qtotal;
qin(18)=1/2*1/6*qtotal;
qin(19)=1/2*2/3*qtotal;

qin(6)=1/4*1/2*qtotal;
qin(7)=1/4*1/2*qtotal;
qin(9)=1/4*1/2*qtotal;
qin(12)=1/4*1/2*qtotal;

qin(20)=1/2*1/6*qtotal;
qin(21)=1/2*1/6*qtotal;
qin(22)=1/2*2/3*qtotal;
...
for i=1:thermalStep
    ...
    Tave1=1/2*1/3*(T(17)+T(19)+T(18))+...
        1/2*1/4*(T(2)+T(3)+T(5)+T(8));
    rot1=Thermal_Timoshenko(Tave1,modelTimoshenkoConstant,...
        modelThermalConstant,creaseWidthMat(3));
    tempHis1(i)=Tave1;
    
    Tave2=1/2*1/3*(T(20)+T(21)+T(22))+...
    1/2*1/4*(T(6)+T(7)+T(9)+T(12));
    rot2=Thermal_Timoshenko(Tave2,modelTimoshenkoConstant,...
        modelThermalConstant,creaseWidthMat(6));   
    tempHis2(i)=Tave2;
    ...
    rotationZeroStrain(3)=pi+ratio*pi+rot1;
    rotationZeroStrain(6)=pi+ratio*pi+rot2;
    modelMechanicalConstant{13}=rotationZeroStrain;
    ...
    node1=squeeze(gravityShape(5,:))'+squeeze(UhisThermal(i,5,:));
    node2=squeeze(gravityShape(6,:))'+squeeze(UhisThermal(i,6,:));
    node3=squeeze(gravityShape(9,:))'+squeeze(UhisThermal(i,9,:));
    node4=squeeze(gravityShape(10,:))'+squeeze(UhisThermal(i,10,:));
    vec1=node2-node1;
    vec2=node4-node3;
    rotation=dot(vec1,vec2)/norm(vec1)/norm(vec2);
    foldHis2(i)=acos(rotation);
...
% figure; hold on; plot(qhis,foldHis); plot(qhis,foldHis2); hold off;
% figure; hold on; plot(foldHis,tempHis1);plot(foldHis,tempHis2); hold off;
