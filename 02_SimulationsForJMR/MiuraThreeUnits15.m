...
n=2;
Ext=0.99888444;
[Node,Panel]=GenerateMiuraSheet(a,b,gama,m,n,Ext);
...
ViewControl(3)=180*10^(-3); % Vsize: displayed axis range 
...
ModelConstant{9}=6; % DiagonalRate:
...
Load=[19,-LoadForce,0,0.51*LoadForce;
      22,-LoadForce,0,0.51*LoadForce;
      20,LoadForce,0,-0.5*LoadForce;
      21,LoadForce,0,-0.5*LoadForce;];

% Load=[19,0,0,+LoadForce;
%       22,0,0,+LoadForce;
%       20,0,0,-0.9*LoadForce;
%       21,0,0,-0.9*LoadForce;];
%   

%% Nonlinear Solver for loading
LoadConstant=zeros(4,1);
LoadConstant(1)=130; % IncreStep
...
[Theta]=plotDeformedHisAndCalcTheta(ViewControl,newNode,newPanel,...
    UhisLoading,UhisAssemble,PanelNum,SprIJKL,AngleNum);
