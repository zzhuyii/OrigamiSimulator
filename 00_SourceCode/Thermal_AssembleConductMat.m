...
function [thermalMat,thermalNodeNum]=...
    Thermal_AssembleConductMat(newNode,newPanel,...
    barConnect,barLength,barType,U,...
    modelThermalConstant,modelMechanicalConstant,...
    ...
    kPanelMat=modelThermalConstant{1};
    kCrease=modelThermalConstant{2};    
    kEnv=modelThermalConstant{3};    
    tEnvMat=modelThermalConstant{4};
    tEnvCrease=modelThermalConstant{5};
    
    envLayer=modelThermalConstant{6};
    thermalDissipationAngle=modelThermalConstant{7}; 
    thermalBoundaryPanels=modelThermalConstant{9}; 
    
    panelThicknessMat=modelMechanicalConstant{5};
    creaseThicknessMat=modelMechanicalConstant{6};
    
    thermalMat=zeros((envLayer+1)*thermalNodeNum);
    maxEnvThickness=1000000*ones(newPanelNum,1);
    ...
    count=1;
    for i=1:newPanelNum
        A=size(thermalBoundaryPanels);
        BCnum=A(1);
        check=0;
        for j=1:BCnum
            if newPanel2OldPanel(i)==thermalBoundaryPanels(j)
                check=1;
            end
        end
        if check==1
    ...
    for i=range
        ...
        nodej=nodeInfo(2);
        nodek=nodeInfo(3);
        pt1=(newNode(nodei,:)+U(nodei,:))';
        pt2=(newNode(nodej,:)+U(nodej,:))';
        pt3=(newNode(nodek,:)+U(nodek,:))';
        ...
        for j=BCindex
            ...
            pointj=pointInfo(2);
            pointk=pointInfo(3);            
            point1=(newNode(pointi,:)+U(pointi,:))';
            point2=(newNode(pointj,:)+U(pointj,:))';
            point3=(newNode(pointk,:)+U(pointk,:))';            
            [D1temp,zone,N1,N2]=Contact_P2TDistance(pt1,point1,point2,point3);
            if D1>D1temp
                D1=D1temp;
            end
        end
        ...
        for j=BCindex
            ...
            point2=(newNode(pointj,:)+U(pointj,:))';
            point3=(newNode(pointk,:)+U(pointk,:))';            
            [D2temp,zone,N1,N2]=Contact_P2TDistance(pt2,point1,point2,point3);
            if D2>D2temp
                D2=D2temp;
            end
        end
        ...
        for j=BCindex
            ...
            point2=(newNode(pointj,:)+U(pointj,:))';
            point3=(newNode(pointk,:)+U(pointk,:))';            
            [D3temp,zone,N1,N2]=Contact_P2TDistance(pt3,point1,point2,point3);
            if D3>D3temp
                D3=D3temp;
            end
        end
    ...
    for i=1:newPanelNum
       ...
       if barType(bar1)==5 || barType(bar2)==5 || barType(bar3)==5
           thick=panelThicknessMat(newPanel2OldPanel(i));
       else
           thick = creaseThicknessMat(max(max(newCrease2OldCrease(bar1),...
               newCrease2OldCrease(bar2)),newCrease2OldCrease(bar3)));
       end
       
       % T3 for assembling the thermal matrix
       point1=(newNode(nodei,:)+U(nodei,:))';
       point2=(newNode(nodej,:)+U(nodej,:))';
       point3=(newNode(nodek,:)+U(nodek,:))';  
       vec1=point2-point1;
       vec2=point3-point1;
       ...
       kmat=kCrease;
       if newPanel2OldPanel(i)~=0
           kmat=kPanelMat(newPanel2OldPanel(i));
       end
       ...
       for q=1:envLayer
           ...
           if newPanel2OldPanel(i)==0
               ...
               if tEnvMat(newPanel2OldPanel(i))<maxEnvThickness(i)
                   airThick=tEnvMat(newPanel2OldPanel(i));
               else
                   airThick=maxEnvThickness(i);
               end
           end
           ...
           Ac=2/3*At;
           Ac=Ac+Lsum*(q-0.5)/envLayer*airThick*2/3*tan(thermalDissipationAngle);
           kic=kEnv*Ac*envLayer/airThick;           
           kjc=kic;
           kkc=kic;
