...
function [Theta]= plotDeformedHisAndCalcTheta(ViewControl,newNode,newPanel,UloadHis,UassembleHis,PanelNum,SprIJKL,AngleNum)
...
for i=1:Incre1
    ...
    [ThetaTemp]=CreaseTheta(tempU,SprIJKL,newNode);   
    Theta(i)=((ThetaTemp(AngleNum(1))+ThetaTemp(AngleNum(2)))/2+ThetaTemp(AngleNum(3))+ThetaTemp(AngleNum(4)))-2*pi();
...
for i=1:Incre2
    ...
    [ThetaTemp]=CreaseTheta(tempU,SprIJKL,newNode);   
    Theta(i+Incre1)=((ThetaTemp(AngleNum(1))+ThetaTemp(AngleNum(2)))/2+ThetaTemp(AngleNum(3))+ThetaTemp(AngleNum(4)))-2*pi();
